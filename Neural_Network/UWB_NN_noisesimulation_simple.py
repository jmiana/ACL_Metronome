...
abs_path = "~/ACL_UWB_Pose_Estimation_Research_Project/Neural_Network/" # modify this as needed
...
def modified_pose_z_noise_distr_creator(poses, noise, data_distro_mode):
    """
    Author: Julianne Miana (jmiana@mit.edu)
    Modifies pose_z_noise_distr_creator to create a new csv file each time function is called,
    avoids the need to rerun file every time data distribution mode changes
    Inputs: poses array, noise array, data_distro_mode string
    Outputs: returns nothing, creates a new csv file in location specified by abs_path
    """
    # Create DataFrame object with titles/headers
    df = pd.DataFrame( columns=data_titles)  # Include the column titles)
    mod_path = abs_path + data_distro_mode + ".csv"
    df.to_csv(mod_path,  mode='a', header=True, index=False)
    ...
    # Append data to existing DataFrame object
    df = pd.DataFrame({data_titles[0]: x_pose,
    data_titles[1]: y_pose,
    data_titles[2]: z_pose,
    data_titles[3]: roll,
    ...
    data_titles[8]: z} , 
    columns=data_titles)  # Include the column titles)
    df.to_csv(mod_path,  mode='a', header=False, index=False)
...
distribution_noise_gaussian_1 = "Normal(mean=0,stdev=0.1)"
modified_pose_z_noise_distr_creator(poses, noise_gaussian_1, distribution_noise_gaussian_1)
...
distribution_noise_uniform_1 = "Uniform(min=-0.1, max=0.1)"
modified_pose_z_noise_distr_creator(poses, noise_uniform_1, distribution_noise_uniform_1)
...
distribution_noise_binomial_1 = "Binomial(n=100, p=0.5)"
modified_pose_z_noise_distr_creator(poses, noise_binomial_1, distribution_noise_binomial_1)
